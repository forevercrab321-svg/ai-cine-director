        ...
        <div className="relative max-w-6xl mx-auto px-6 py-20">
          ...
          <div className="text-center mb-16">
            ...
            <p className="text-sm text-slate-500 mt-4">
              {lang === 'zh' ? '无需信用卡，立即开始' : 'No credit card required'}
            </p>
          </div>
          ...
          <div className="max-w-4xl mx-auto mb-20">
            ...
            <p className="text-center text-slate-500 text-sm mt-4">
              {lang === 'zh' ? '由 AI Cine-Director 生成的示例作品' : 'Sample work generated by AI Cine-Director'}
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {BUSINESS_PLANS.map((plan, index) => (
            <div key={plan.id} className={`bg-slate-900/50 p-6 rounded-xl border ${plan.popular ? 'border-indigo-500/50 bg-indigo-900/20' : 'border-slate-800'} text-center relative`}>
              ...
              <p className="text-3xl font-bold text-indigo-400 mb-4">${plan.priceMonthly}<span className="text-sm text-slate-400">/月</span></p>
              <p className="text-slate-400 text-sm mb-4">{plan.creditsMonthly.toLocaleString()} {lang === 'zh' ? '积分/月' : 'credits/month'}</p>
