...
export function buildShotImagePrompt(input: ShotPromptCompilerInput): CompiledShotPrompt {
  ...
  // image_prompt is the per-shot Gemini-generated visual description — must be first candidate
  // so every shot drives its own unique image instead of falling back to the shared scene description
  ...
  // image_prompt from shot takes priority — it is the unique per-shot visual description
  // generated by Gemini during director-brain planning. Surface it first so the model
  // treats it as the definitive frame directive, not the generic scene summary.
  const perShotImagePrompt = shot.image_prompt ? String(shot.image_prompt).trim() : '';

  const storyBlock = [
    perShotImagePrompt ? `THIS SHOT: ${perShotImagePrompt}` : '',
  ...
  const technicalPolish = [
    ...
    'Prioritize this shot\'s specific image_prompt over any generic scene template',
    'Render as a single coherent film frame — not a collage or split screen',
  ].join('. ');

  // CONSISTENCY MANDATE — runs for every shot in the film.
  // Ensures character identity, wardrobe, and visual style are locked across the entire
  // 12-panel storyboard batch so the output reads as one continuous film, not isolated images.
  const consistencyParts: string[] = [];
  if (input.characterAnchor) {
    consistencyParts.push(`CHARACTER IDENTITY LOCK — preserve exact appearance: ${input.characterAnchor}`);
  }
  if (styleBible?.color_palette) {
    consistencyParts.push(`COLOUR PALETTE LOCK: ${styleBible.color_palette}`);
  }
  if (styleBible?.lens_language) {
    consistencyParts.push(`LENS & GRADE LOCK: ${styleBible.lens_language}`);
  }
  if (styleBible?.art_direction) {
    consistencyParts.push(`ART DIRECTION: ${styleBible.art_direction}`);
  }
  if (input.styleLabel) {
    consistencyParts.push(`VISUAL STYLE: ${input.styleLabel}`);
  }
  if (characters.length > 0) {
    consistencyParts.push(`CHARACTERS PRESENT — must match established look: ${characters.join(', ')}`);
  }
  const consistencyBlock = consistencyParts.length > 0
    ? consistencyParts.join('. ')
    : 'Maintain full visual continuity with the established film style';

  const modelPrompt = [
    // Consistency mandate first — highest-weight signal for the model
    `FILM CONSISTENCY MANDATE: ${consistencyBlock}`,
    storyBlock,
    cameraBlock,
